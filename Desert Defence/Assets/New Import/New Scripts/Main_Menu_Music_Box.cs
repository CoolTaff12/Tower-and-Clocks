...
public class Main_Menu_Music_Box : MonoBehaviour 
{
	...
	{	
		...
		if(M1)
		{
			if (level == 10 || level == 11 || level == 12 || level == 13 || level == 3)									// CPU Mode with Mouse
			{
		...
		if(M3)
		{
			if (level == 12 || level == 13 || level == 3)									// CPU Mode with Mouse
			{
				Destroy (gameObject);												// Destroys the object when the specific level is loaded
			}
			else
		...
		if(M4)
		{
			if (level == 13 || level == 3)									// CPU Mode with Mouse
			{
				Destroy (gameObject);												// Destroys the object when the specific level is loaded
			}
			else
